...
<body style="background: url('{{ asset('assets/img/bgpos.jpg') }}')">
                    ...
                    <div class="offcanvas-body ms-lg-auto d-flex flex-column h-100">
                        <ul class="navbar-nav">
                            <li class="nav-item dropdown">
                                <a class="nav-link dropdown-toggle fw-bolder" href="#" data-bs-toggle="dropdown">Hi, Penpos Bahan Baku Lokal!</a>
                        ...
                        <table class="table table-centered table-nowrap" style="width: 100%">
                            <thead class="thead-dark"
                                ...
                                <tr>
                                    ...
                                    <th class="border-0 text-center" style='width:25%'>Bahan Baku</th>
                                    <th class="border-0 text-center" style='width:30%'>Jumlah (paket)</th>
                                    <th class="border-0 rounded-end text-center" style='width:20%'>Harga /paket</th>
                                    <th class="border-0 rounded-end text-center" style='width:20%'>Tersedia</th>
                                </tr>
                            </thead>
                            <tbody style="display: block; overflow-y: auto; height: 70vh;">
                                ...
                                <tr style='display: table; table-layout: fixed; width: 100%'>
                                    ...
                                    <input type="hidden" class="local-price" id="local-price-{{ $i->id }}"
                                        value="{{ $i->local_price }}">

                                    <td class="text-center" style='width:5%'>{{ $index++ }}</td>
                                    <td class="text-center" style='width:25%' id="ingredient-name-{{ $i->id }}">{{ $i->name }}</td>
                                    ...
                                    <td class="text-center" style='width:20%'>{{ $i->local_price }} TC</td>
                                    <td class="text-center" style='width:20%' id="ingredient-local-{{ $i->id }}">
    ...
    <script type="text/javascript">
        ...
        const updateIngredientPriceAndLimit = () => {
            ...
            let ingredientsLocalPrice = $(`.local-price`).map(function() { return $(this).val() }).get()

            // Update Total
            let totalPrice = 0
            for (let i = 0; i < ingredientsAmount.length; i++) {
                totalPrice += (ingredientsAmount[i] * ingredientsLocalPrice[i])
            }
        ...
        const buyIngredients = () => {
            ...
            let ingredientType = ingredientId.map(id => { return "local" })
            let teamId = $(`#pilih-tim`).val()
        ...
        window.Echo.channel('update-import').listen('.import', (e) => {
            e.imports.forEach(ingredient => {
                $(`#ingredient-local-${ingredient.id}`).text(`${ingredient.amount} Paket`)
            })
        })
